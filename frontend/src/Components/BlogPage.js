    ...
    render() {
        ...
        return (
            <>
                <ScrollToTop/>
                <Helmet>
                    <meta name="description" content={textAreaArray[1]} />
                    <meta name="keywords" content={textAreaArray[2]} />
                    <meta name="author" content={textAreaArray[3]}/>

                    <title>{this.props.blog.blogTitle}</title>
                    <link rel="canonical" href={this.context.webAddress + this.props.blog.url} />
                    ...
                    <article className="blog-post px-3 py-5 p-md-5" style={{ marginBottom: "50px" }}>
                        <div className="container single-col-max-width">
                            <header className="blog-post-header">
                                <h1 className="title mb-2">{this.props.blog.blogTitle}</h1>
                                <div className="meta mb-3">
                                    <span className="dateBlog" style={{ fontFamily: "sans-serif" }}>Published on {moment(this.props.blog.date).format('DD MMM YYYY')}</span>
                                    <span className="comment" style={{ fontFamily: "sans-serif" }}>{this.props.blog.totalView} views</span>
                                </div>
                            </header>
                                    ...
                                    return (
                                        <React.Fragment key={position * index * 6985 + position * 985}>
                                            ...
                                            {(tagArray[position] === "code") ?
                                                ...
                                                : null}
                                            {(tagArray[position] === "code_output") ?
                                                <>
                                                    <SyntaxHighlighter language={textAreaArray[position][2]} style={a11y_light}>
                                                        {textAreaArray[position][1]}
                                                    </SyntaxHighlighter>
                                                </>
                                                : null}
                                            ...
                                            {(tagArray[position] === "img") ?
                                                <figure className="blog-banner">
                                                    <a href={`${this.context.webAddress}${textAreaArray[position][1]}`} target="_blank" rel="noopener noreferrer">
                                                        <img className="img-fluidBlog" src={`${this.context.webAddress}${textAreaArray[position][1]}`} alt={textAreaArray[position][2]} />
                                                    </a>
                                                </figure>
                                                : null}
                                            ...
                                            {(tagArray[position] === "pdf") ?
                                                <div className="mt-4">
                                                    <PdfViewer pdf={`${this.context.webAddress}${textAreaArray[position]}`} />
                                                </div>
                                                : null}
                            ...
                            <nav className="blog-nav nav nav-justified my-5">
                                <Link  className="nav-link-prev nav-item nav-link rounded-left" to={( (typeof textAreaArray[4] === "undefined") || (textAreaArray[4] === "") ) ?"/blog": textAreaArray[4]}>
                                    Previous
                                ...
                                <Link className="nav-link-next nav-item nav-link rounded-right" to={( (typeof textAreaArray[5] === "undefined") || (textAreaArray[5] === "") ) ?"/blog": textAreaArray[5]}>
                                    Next
